import argparse
import json
import tabling_annealer, officer_annealer
import random
import numpy as np
...
class Scheduler():
    ...
    def __init__(self, json_string, outfile):
        self.data = json.load(open(json_string))["data"]
    ...
    def write_output(self, schedule, number_of_slots):
        ...
        for day in range(number_of_days):
            ...
            for slot in range(number_of_slots // number_of_days):
                out_schedule[day].append(schedule[day*number_of_days + slot])
    ...
    def run_member_scheduling(self, officer_schedule, initial_schedule, availabilities, overlaps):
        ...
        member_scheduler = tabling_annealer.TablingAnnealer([member for member in self.member_list if member not in officer_schedule], initial_schedule[:], availabilities, overlaps, self.max_people_per_slot)
        Tmax = 10.0
        Tmin = 0.1
        steps = 50000
